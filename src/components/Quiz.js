...
function Quiz() {
    ...
    return (
        <div className="quiz">
            <p>Questions answered: {questionsAnswered} / {questions.length}</p>
            <p>Time remaining: {Math.floor(timeLeft / 60)}:{timeLeft % 60 <10 ?`0${timeLeft % 60}` :timeLeft % 60}</p>
            {showResult ? (
                ...
                <div>
                    ...
                    <button className="submit-button" onClick={handleSubmit} disabled={timeLeft <= 0}>
                        Submit
                    </button>
                </div>
            )}
